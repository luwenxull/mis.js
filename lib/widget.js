...
function findParentWithDom(ele) {
    while (ele) {
        if (ele.parent) {
...
function findSiblingDom(ele) {
    let parentWithDom = findParentWithDom(ele), sibling = null;
    walk(ele, next => {
        if (next === parentWithDom)
            return true;
...
export function makeList(initialList, render, key, equal) {
    let list = [];
